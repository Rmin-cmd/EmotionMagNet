...
class ChebNet(nn.Module):
    ...
    def forward(self, real, imag, graph, layer=2):
        ...
        her_mat = np.array([decomp(data, self.q, norm=True, laplacian=True, max_eigen=2, gcn_appr=True)
                            for data in graph.numpy()])

        cheb_graph = torch.tensor([cheb_poly(her_mat[i], self.K) for i in range(her_mat.shape[0])]).to(device)
